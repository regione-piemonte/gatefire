...
public class FiscalCodeUtils implements Serializable {
	...
	private static final int[] SETDISP = { 1, 0, 5, 7, 9, 13, 15, 17, 19, 21, 2, 4, 18, 20, 11, 3, 6, 8, 12, 14, 16, 10,
			22, 25, 24, 23 };
	...
	public static FiscalCodeParserResult parseFiscalCode(String cf) {
		...
		if (isEmpty(cf) || cf.length() != 16) {
			...
			result.setMessage("Invalid FiscalCode lenght");
			return result;
		}
		...
		for (int i = 0; i < 6; i++) {
			if (!(Character.isLetter(cf.charAt(i)))) { // controllo dei primi 6
				result.setResultCode(2);
				result.setMessage("Invalid Name or Surname");
				return result;
			}
		}

		// Controllo dei campi numerici che possono essere numeri oppure lettere
		if (!validateDigits(cf)) {
			result.setResultCode(3);
			result.setMessage("Could not Validate Digit Character");
			return result;
		}
		...
		try {
			...
			result.setMessage("Invalid Year");
		...
		try {
			...
			if (month != null) {
				...
				result.setMessage(INVALID_MONTH);
				return result;
			}
		} catch (Exception e) {
			result.setMessage(INVALID_MONTH);
		...
		try {
			...
			int day = Integer.parseInt(getDigit(dd.charAt(0)) + "" + getDigit(dd.charAt(1)));
			if (day > 40) {
				male = false;
				day -= 40;
			}
		...
		try {
			...
			result.setMessage("Invalid Birthday");
			result.setResultCode(7);
			return result;
		}

		// Validazione Primo Carattere codice Istat del Comune. Deve essere una
		// Lettera tra quelle in elenco
		...
		// controllo del 1° carattere
			...
			result.setMessage("Invalid Istat Code Character");
			return result;
		}
		int s = 0;
		...
		if (s % 26 + 'A' != cf.charAt(15)) {
			...
			result.setMessage("Invalid fiscal code control character");
			return result;

		}
		result.setBirthDay(birthDay);
		result.setBirthCountry(getBirthCountry(cf));
	...
	private static boolean validateDigits(String cf) {
		...
		for (int i = 0; i < digits.length(); i++) {
			...
			if (!(Character.isDigit(currentChar) || DIGITCONVERSIONMAP.containsKey(currentChar))) {
				return false;
			}
		}
		return true;
	...
	private static String getBirthCountry(String cf) {
		...
		result = result.charAt(0) + "" + getDigit(result.charAt(1)) + "" + getDigit(result.charAt(2)) + ""
	...
	public static class FiscalCodeParserResult implements Serializable {
		/**
		 *
		 */
		private static final long serialVersionUID = 1L;
		...
		public String toString() {
			return "FiscalCodeParserResult [birthDay=" + birthDay + ", resultCode=" + resultCode + ", male=" + male
					+ ", birthCountry=" + birthCountry + ", message=" + message + "]";
		}
